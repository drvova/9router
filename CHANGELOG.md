...
- **WARP**: add managed Cloudflare Zero Trust enrollment and free consumer registration flows, including browser login, official local proxy discovery, and proxy-pool integration
- **WARP**: bundle reproducible native egress helper builds and support cross-platform targets including Linux 386, ARM, PPC64LE, and s390x
...
- **WARP**: log QUIC dial failures in the proxy handler instead of returning an opaque upstream error
- **WARP**: retry proxy readiness checks when the tunnel has not finished starting, and detect dead helper processes before returning cached proxy URLs
- **Build**: remove remaining webpack source and configuration paths in favor of Turbopack
