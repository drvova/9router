...
- **Providers**: Anthropic Messages dual transports for 12 providers whose native `/v1/messages` endpoints were verified live (auth-error + control-404 probes): Moonshot, LongCat, Novita, Synthetic, ModelScope, NanoGPT, StepFun, AIMLAPI, Requesty, SiliconFlow, iFlow, xAI — Claude-format clients now skip the lossy OpenAI translation hop
