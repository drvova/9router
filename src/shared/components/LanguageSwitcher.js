...
import { getCurrentLocale, onLocaleChange, reloadTranslations } from "@/i18n/runtime";
import { getLocaleInfo } from "@/shared/constants/locales";
...
export default function LanguageSwitcher({ className = "", isOpen: controlledOpen, onClose, hideTrigger = false }) {
  ...
  // Rendered into document.body so the modal escapes any parent layout constraints.
  const modal = isOpen
    ? createPortal(
  ...
  // With the trigger hidden this component contributes nothing to the layout,
  // so return the portal directly rather than an empty wrapper element.
  if (hideTrigger) return modal;
