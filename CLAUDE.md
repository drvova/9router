...
cp .env.example .env
npm install
npm run dev                                                          # dev on port 20128
npm run build && PORT=20128 HOSTNAME=0.0.0.0 npm run start           # production
```
- Bun variants: `npm run dev:bun` / `build:bun` / `start:bun`.
- **Bundler.** All development and production builds use Turbopack. `next build` writes the configured standalone output; use `NEXT_DIST_DIR` when building while a dev server is active so the dev cache is not overwritten. `cli/scripts/build-cli.js` already does this correctly (`NEXT_DIST_DIR=.next-cli-build`).
...
> - **`known-fails.txt` is environment-specific**, curated where `cloud/` exists and credentials are present. On a bare local checkout ~86 assertions fail while only ~14 match the baseline, so the gate is honest but cannot go green here — read its output as a diff, not a verdict, and do **not** re-snapshot the baseline locally or you will launder ~60 real failures into "expected". To judge whether *your own change* regressed anything, run the suite before and after on the same machine and compare the failure sets; a test that passed before and fails now is the only regression that matters.
