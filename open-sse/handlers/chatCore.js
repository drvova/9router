...
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, rtkEnabled, headroomEnabled, headroomUrl, headroomCompressUserMessages, cavemanEnabled, cavemanLevel, ponytailEnabled, ponytailLevel, pxpipeEnabled, pxpipeMinChars, pxpipeTimeoutMs, pxpipeTransform, onPxpipeEvent, sourceFormatOverride, providerThinking, providerSystemPrompt, providerSystemPromptVars }) {
  ...
  const targetFormat = modelTargetFormat || runtimeTransport?.format
    || getTargetFormat(provider, credentials?.providerSpecificData?.apiType);
  ...
  if (tokenSaverEnabled && cavemanEnabled && cavemanLevel) {
    translatedBody = injectCaveman(translatedBody, finalFormat, cavemanLevel);
    xf.push(`CAVEMAN:${cavemanLevel}`);
  }

  // Ponytail: inject lazy-senior-dev system prompt
  if (tokenSaverEnabled && ponytailEnabled && ponytailLevel) {
    translatedBody = injectPonytail(translatedBody, finalFormat, ponytailLevel);
  ...
  // Handle 401/403 - try token refresh (skip for noAuth providers, and for executors
  // that cannot refresh: retrying a deterministic null only adds latency to the failure)
  if (!executor.noAuth && executor.canRefresh?.(credentials) !== false
